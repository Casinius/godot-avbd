...
inline void solve(float3x3 aLin, float3x3 aAng, float3x3 aCross, float3 bLin, float3 bAng, float3 &xLin, float3 &xAng) noexcept
    {
        ...
        // Build 6x6 symmetric matrix [aLin  aCross^T; aCross  aAng] from custom structs
        Eigen::MatrixXf A(6, 6);
        // Top-left: aLin (lower triangle only)
        A << aLin(0,0), 0.0f, 0.0f,
             aLin(1,0), aLin(1,1), 0.0f,
             aLin(2,0), aLin(2,1), aLin(2,2),
             // Top-right: aCross^T (transpose of aCross lower triangle)
             aCross(0,0), aCross(1,0), aCross(2,0),
             // Bottom-left: aCross (lower triangle only)
             aCross(0,1), aCross(1,1), 0.0f,
             aCross(0,2), aCross(1,2), aCross(2,2),
             // Bottom-right: aAng (lower triangle only)
             aAng(0,0), 0.0f, 0.0f,
             aAng(1,0), aAng(1,1), 0.0f,
             aAng(2,0), aAng(2,1), aAng(2,2);
        ...
        xLin[0] = x(0, 0); xLin[1] = x(1, 0); xLin[2] = x(2, 0);
        xAng[0] = x(3, 0); xAng[1] = x(4, 0); xAng[2] = x(5, 0);
    }
